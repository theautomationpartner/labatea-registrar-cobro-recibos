...
import type { DocumentosEmision, FormatoResumen } from '@/types'
...
interface VerPdfResumenProps {
  ...
  /** Cómo salió cada documento en la emisión: sólo se ofrece el PDF de los que salieron. */
  documentos: DocumentosEmision | null
}

/**
 * Los botones para ABRIR EN UNA PESTAÑA el PDF de cada documento recién emitido, y desde ahí
 ...
 */
export function VerPdfResumen({ ctaCteId, formato, documentos }: VerPdfResumenProps) {
  const [abriendo, setAbriendo] = useState<DocumentoArchivo | null>(null)
  const [error, setError] = useState<string | null>(null)

  if (!ctaCteId || !documentos || (formato !== 'PDF' && formato !== 'Ambos')) return null
  const disponibles = (['resumen', 'estado'] as const).filter((d) => documentos[d] === 'ok')
